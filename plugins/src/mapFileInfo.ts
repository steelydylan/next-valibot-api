...
function convertToValidVariableName(input: string): string {
  ...
  validVariableName = validVariableName.replace(
    /_([a-zA-Z0-9])/g,
    (match, p1) => {
  ...
  // 最初の文字を大文字に
  validVariableName =
    validVariableName.charAt(0).toUpperCase() + validVariableName.slice(1);

  return validVariableName;
}

export function mapFileInfo(dist: string, appDir: string, program: ts.Program) {
